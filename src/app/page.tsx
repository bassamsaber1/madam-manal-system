  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err instanceof Error && err.name === 'AbortError') {
        setSystemError('البحث أخذ وقت طويل — جرب Facebook ID بدل رقم الموبايل');
      } else {
              ...
              <form onSubmit={handleSearch} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    disabled={loading}
                    className="w-full px-4 py-3.5 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 font-bold text-center text-slate-800 text-base disabled:opacity-60"
                    inputMode="search"
                    autoComplete="off"
                    required
                  />
                  <p className="mt-2 text-xs text-slate-400 text-center">
                    بحث مباشر من ALL.txt — قد يأخذ دقائق
                  </p>
                </div>
                <button
                  type="submit"
